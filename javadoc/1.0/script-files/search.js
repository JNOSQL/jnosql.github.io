...
function findMatch(matcher, input, startOfName, endOfName, prefixLength) {
  ...
  for (var i = 1; i < match.length; i += 2) {
    ...
    if (start !== 0 && start !== startOfName) {
      if (
        ...
        if (
          !isUpperCase(input[start]) ||
          (!isLowerCase(prevChar) && !isLowerCase(nextChar))
        ) {
  ...
  if (prefixLength < match.index && prefixLength < startOfName) {
    ...
    score -=
      rateDistance(
        input.substring(prefixLength, Math.min(match.index, startOfName))
      ) * factor;
  }
...
function doSearch(request, response) {
  ...
  function searchIndex(indexArray, category) {
    ...
    $.each(indexArray, function (i, item) {
      ...
      var endOfName =
        category === "members" && qualName.indexOf("(", startOfName) > -1
          ? qualName.indexOf("(", startOfName)
          : qualName.length;
      var m = findMatch(
    ...
    return matches
      .sort(function (e1, e2) {
        return (
          e2.score - e1.score ||
          (category !== "members" ? e1.name.localeCompare(e2.name) : 0)
        );
      })
      .slice(0, maxResults);
  }
...
$.widget("custom.catcomplete", $.ui.autocomplete, {
  ...
  _renderMenu: function (ul, items) {
    ...
    ul.append(
      "<li class='ui-static-link'><div><a href='" +
        pathtoroot +
        "search.html?q=" +
...
$(function () {
  ...
  search.val("");
  search.on("input", () =>
    reset.css("visibility", search.val() ? "visible" : "hidden")
  );
