...
function makeFilterWidget(sidebar, updateToc) {
  ...
  function handleTocFocus(event) {
    let links = Array.from(sidebar.querySelectorAll("ol > li > a")).filter(
      (link) => link.offsetParent && link.tabIndex === 0
    );
  ...
  function expand() {
    ...
    if (tocMenu) {
      ...
      expandedHeight = Math.min(
        ...
        Math.max(
          expandedHeight,
          tocMenu.querySelector("div.toc-header").offsetHeight +
            tocMenu.querySelector("ol.toc-list").scrollHeight +
            10
        )
      );
